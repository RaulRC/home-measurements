...
def get_random_colors():
    ...
    return COLOR
def plot_ts_value(df, value='temperature'):
    fig = px.line(df, x='timestamp', y=value, color='room', title=value, color_discrete_sequence=get_random_colors())
...
with tab1:
    ...
    with col1:
        st.plotly_chart(plot_ts_value(df[df['room'] == 'basement'].sort_values(by='timestamp'), value='humidity'))
        st.plotly_chart(plot_box_value(df[df['room'] == 'basement'].sort_values(by='timestamp'), value='humidity'))
        st.write(df.sample(100).reset_index()[MAIN_COLS])
    with col2:
        st.plotly_chart(plot_ts_value(df[df['room'] == 'basement'].sort_values(by='timestamp'), value='temperature'))
